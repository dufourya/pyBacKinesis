...
class BoxExponential:
    ...
    def __init__(self, SampleNumber, SampleInterval):
        ...
        self.Dimensions = np.array([5e3, 1e3, 1e3], dtype = np.int16)
        self.WorldScaling = 1
    ...
    def update(self, CellBody, SimulationTimeStep):
        ...
        self.update_consumption(CellBody, SimulationTimeStep)
    ...
    def update_consumption(self, CellBody, SimulationTimeStep):
        CellBody.LigandConsumed = SimulationTimeStep * (CellBody.LigandConsumptionRate[None,:] * \
            (1 / (1 + CellBody.LigandMichaelisConstant[None,:] / self.get_ligand_concentration(CellBody))) \
                - CellBody.LigandSecretionRate[None,:])
        CellBody.LigandConsumedIntegrated += CellBody.LigandConsumed

class BoxLinear:

    def __init__(self, SampleNumber, SampleInterval):
        self.GradientSlope = np.array([10, 1])
        self.LigandBackgroundConcentration = np.array([1, 10])
...
class StepLigand:
    ...
    def get_ligand_concentration(self, CellBody):
        LigandConcentration = np.ones(CellBody.CellNumber)[:,None] * self.LigandConcentration[None,:]
        return LigandConcentration
...
class Maze_and_Diffusion:
    ...
    def __init__(self, SampleNumber, SampleInterval):
        ...
        self.WorldScaling = 5 #scale gradient matrix to speed up reaction-diffusion updates
        self.DimensionsScaled = np.uint16(self.Dimensions / self.WorldScaling)
        ...
        self.LigandInitialConcentration = np.array([1000, 1e6])
        self.LigandDiffusionCoefficient = np.array([5e2, 3e2])
    ...
    def update(self, CellBody, SimulationTimeStep):
        self.update_consumption(CellBody, SimulationTimeStep)
        self.update_diffusion(SimulationTimeStep)
    ...
    def check_world_boundaries(self, CellBody):
        ...
        CellBody.Position[Collisions,:2] = self.WorldScaling * (rng.random((np.sum(Collisions),2)) + \
            self.ObstaclesIndices[:, CellBody.PositionScaledWorld[Collisions,0], CellBody.PositionScaledWorld[Collisions,1]].T)
    ...
    def update_consumption(self, CellBody, SimulationTimeStep):
        CellBody.LigandConsumed = SimulationTimeStep * (CellBody.LigandConsumptionRate[None,:] * \
            (1 / (1 + CellBody.LigandMichaelisConstant[None,:] / self.LigandConcentration[CellBody.PositionScaledWorld[:,0], CellBody.PositionScaledWorld[:,1], CellBody.PositionScaledWorld[:,2], :])) \
                - CellBody.LigandSecretionRate[None,:])
    ...
    def update_diffusion(self, SimulationTimeStep):
        for k in range(self.LigandNumber):
...
class Maze_and_Exponential:
    ...
    def __init__(self, SampleNumber, SampleInterval):
        self.Dimensions = np.array([10000, 5000, 1000])
        self.WorldScaling = 5 #scale gradient matrix to speed up reaction-diffusion updates
        self.DimensionsScaled = np.uint16(self.Dimensions / self.WorldScaling)
        self.Obstacles = np.zeros(self.DimensionsScaled[:-1], dtype = np.bool)
        self.ObstaclesIndices = np.zeros(((np.ndim(self.Obstacles),) + self.Obstacles.shape), dtype=np.int32)
        self.ObstaclesDensity = 0.2
        self.ObstaclesSize = 500
        # self.initialize_square_positions()
        # self.initialize_circle_positions()
        self.initialize_u_shape_positions()
        # self.initialize_cross_shape_positions()
